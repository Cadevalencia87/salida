...
/* ------------------------------------------------------------
   CONFIGURACIÓN OPCIONAL DE ENVÍO
   Si quieres que las respuestas te lleguen por correo sin
   backend, crea una cuenta gratis en Web3Forms o Formspree,
   pega aquí tu endpoint/llave pública (NUNCA una llave privada)
   y pon ENVIO_ACTIVADO en true. Instrucciones completas en el
   README, sección "Conectar el envío de respuestas".
   ------------------------------------------------------------ */
const ENVIO_ACTIVADO = false;
const ENVIO_ENDPOINT = ""; // ejemplo Web3Forms: "https://api.web3forms.com/submit"
const ENVIO_LLAVE_PUBLICA = ""; // tu access_key pública, nunca una llave privada
...
function confirmarSalida() {
  if (ENVIO_ACTIVADO && ENVIO_ENDPOINT) {
    enviarRespuestas(estado.datos).catch(() => {
      // Si el envío falla, no interrumpimos la experiencia de ella.
      // El detalle queda solo en la consola para quien revise esto luego.
      console.warn("No se pudieron enviar las respuestas al servicio externo.");
    });
  }
  irAPaso("final");
}
...
async function enviarRespuestas(datos) {
  const cuerpo = {
    access_key: ENVIO_LLAVE_PUBLICA, // Web3Forms usa este nombre de campo
    asunto: "Nueva confirmación de salida",
    fecha: datos.fecha,
